...
// - Context is attempt input: the Host supplies one immutable snapshot and
//   each Processor receives only the forms its resolved capabilities support.
...
const CONTEXT_FORMS = ['terms', 'passages']
...
// Capability filtering: each Processor receives only the Context forms its
// resolved role supports. Unsupported forms are never validation errors.
export function filterContext (context, capabilities) {
  ...
  const filtered = { terms: [], passages: [] }

  for (const form of CONTEXT_FORMS) {
    if (supported.includes(form)) {
      filtered[form] = [...snapshot[form]]
    }
  }
...
// Missing arrays normalize to empty; entries are trimmed, empties removed,
// duplicates removed preserving first occurrence. A non-array form or
// non-string entry rejects the snapshot before Provider creation and I/O.
export function normalizeContext (context) {
  const snapshot = { terms: [], passages: [] }
  if (!context) { return snapshot }

  for (const form of CONTEXT_FORMS) {
    const value = context[form]
    if (value === undefined || value === null) { continue }
    if (!Array.isArray(value)) {
      throw processingError('configuration', `Context form "${form}" must be an array`)
    }
    const seen = new Set()
    const entries = []
    for (const entry of value) {
      if (typeof entry !== 'string') {
        throw processingError('configuration', `Context form "${form}" must contain strings`)
      }
...
function contextFormsInUse (context, capabilities) {
  ...
  const forms = []
  for (const form of CONTEXT_FORMS) {
    if (supported.includes(form) && snapshot[form].length > 0) {
      forms.push(form)
    }
  }
  return forms
