...
export const openaiProvider = {
  ...
  create (role, config, secrets, runtime) {
    if (!secrets.key) {
      throw protocolError('configuration', 'An OpenAI API key is required to create a processor')
    }
...
export const openaiCompatibleProvider = {
  ...
  create (role, config, secrets, runtime) {
    if (!secrets.key) {
      throw protocolError('configuration', 'An API key is required to create a processor')
    }
...
class OpenAICompatibleProcessor {
  ...
  async process ({ input, context, instructions, signal }) {
    if (input.kind !== 'text') {
      throw protocolError('configuration', `${this._label} refine requires text input`)
    }
    ...
    if (!text.trim()) {
      throw protocolError('no-text', `${this._label} refine returned no text`)
    }
  ...
  async _send (requestBody, signal) {
    const response = await this._runtime.transport.send({
      method: 'POST',
      url: `${this._config.endpoint.replace(/\/+$/, '')}/chat/completions`,
      headers: {
