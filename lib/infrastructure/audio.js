...
// filename-safe shape (YYYYMMDDTHHMMSSZ) matches ISO 8601's compact form.
// Two captures cannot start in the same second in practice; if the clock
// says otherwise, a counter suffix keeps the file unique rather than
// truncating the earlier recording.
let lastIdSecond = 0
let sameSecondCounter = 0

export function recordingIdForNow () {
  const now = new Date()
  const second = Math.floor(now.getTime() / 1000)
  if (second === lastIdSecond) {
    sameSecondCounter++
  } else {
    lastIdSecond = second
    sameSecondCounter = 0
  ...
  return sameSecondCounter === 0 ? base : `${base}-${sameSecondCounter}`
}
