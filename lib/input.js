...
export class TextPaster {
  ...
  // Locks the intended paste target: the window focused when recording ends.
  // Must be called before processing starts (before write()).
  ...
  set onFocusMismatch (handler) {
    this._onFocusMismatch = handler
  }

  setOnFocusMismatch (handler) {
    this.onFocusMismatch = handler
  }
