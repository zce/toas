...
export class HistoryBrowser {
  ...
  _openDetail (entry) {
    ...
    if (failed && !audio.available) {
      const note = new St.Label({
        text: 'Retry is unavailable: the recording for this session was removed.',
        style_class: 'toas-history-meta'
      })
  ...
  async _runRetry (entry) {
    ...
    if (attempt?.status === 'ok') {
      this._notifier.notify(
        'Retry succeeded',
        'The reprocessed text is on the session detail — open Recent sessions to copy it.'
      )
    } else if (attempt?.status === 'error') {
      this._notifier.notify(
        'Retry failed',
        attempt.error?.message ?? 'The session failed again. Its recording is kept for another try.'
      )
    }
  }
}
