...
export class AudioRecorder {
  ...
  async start () {
    ...
    if (this._format !== 'wav') { throw new Error(`Unsupported recording format: ${this._format}`) }
  ...
  async stop () {
    ...
    if (!process) { throw new Error('Audio capture is not running') }
    ...
    if (this._totalBytes < MIN_PCM_BYTES) {
      this._discardFile()
      throw new Error('Recording must be at least 1 second')
    }
