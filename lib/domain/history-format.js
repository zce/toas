...
// Provider chain from the physical Trace of the calls that actually ran.
// Never inferred from endpoint strings; unknown means no Trace was retained.
export function getProviders (entry) {
  if (!Array.isArray(entry.trace) || entry.trace.length === 0) {
    return 'Unknown'
  }
  const providers = entry.trace
