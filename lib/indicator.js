...
export class ToasIndicator extends PanelMenu.Button {
  ...
  constructor ({ onToggle, onCancel, onClearHistory, onOpenPreferences, onBrowseHistory }) {
    super(0.5, 'toas')
    ...
    this._onBrowseHistory = onBrowseHistory
    ...
    this._historyItem = new PopupMenu.PopupMenuItem('Recent sessions')
    addMenuIcon(this._historyItem, 'document-open-recent-symbolic')
    this._historyItem.connect('activate', () => {
      this.menu.close()
      this._onBrowseHistory?.()
    })
  ...
  render (state) {
    ...
    // Extension-owned class instead of GNOME Shell's internal
    // screen-recording-indicator, which is an unstable implementation detail.
    if (recording) {
      this._icon.add_style_class_name('toas-recording')
    } else {
      this._icon.remove_style_class_name('toas-recording')
    }

    // Accessibility: make the state legible without vision.
    this._icon.set_accessible_name?.(recording ? 'Recording voice input' : 'Voice input')
    ...
    this._historyItem.setSensitive(idle)
  }
