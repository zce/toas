...
export class MimoChatTranscriber {
    ...
    async transcribe(recording, model = this.model) {
        ...
        const transcript = parseTranscript(responseText);
        if (!transcript)
...
function parseTranscript(responseText) {
    const parts = [];
    ...
    for (const rawLine of responseText.split('\n')) {
        ...
        try {
            ...
            throw new Error('Transcription returned malformed SSE');
        }
        ...
        if (event?.error) {
            const message = typeof event.error === 'string'
                ? event.error
                : event.error.message;
            throw new Error(message || 'Transcription stream returned an error');
        }
    ...
    if (sawEventStream && !sawDone)
        throw new Error('Transcription stream ended before [DONE]');
    if (sawEventStream)
    ...
    try {
        ...
        throw new Error('Transcription returned invalid JSON or SSE');
    }
}
