        ...
        try {
            ...
            if (this._refiner.enabled) {
                ...
                try {
                    ...
                    console.warn(
                        `[Voice Prompt] Refine failed, using transcript: ${error.message}`
                    );
                    run.output = run.transcript;
                }
    ...
    _saveHistory(run, status, error = null) {
        try {
            this._history.append({
                ...
                status,
                audio: `recordings/${run.recording.id}.wav`,
            ...
            console.error(
                `[Voice Prompt] Could not save history: ${historyError.message}`
            );
            return false;
        }
    }
