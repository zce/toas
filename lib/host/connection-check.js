...
// through the same resolution pipeline the Kernel uses (resolveRole), then
...
import { resolveRole, processingError } from '../kernel/process.js'
...
export async function runConnectionTest ({ configService, providers, role }) {
  ...
  const roleConfig = role === 'primary' ? config.primary : {
    provider: config.refine.provider,
    endpoint: config.refine.endpoint ?? null,
