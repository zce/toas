...
// - custom-terms: Host-owned Context data source (not part of Config)
//
...
export class ConfigService {
  ...
  // Custom Terms are a Host data source for constructing the attempt
  // Context snapshot; they are not processing configuration.
  snapshotCustomTerms () {
    const raw = this._settings.get_strv?.('custom-terms') ?? []
    const seen = new Set()
    const terms = []
  ...
  snapshotContext () {
    return { terms: this.snapshotCustomTerms(), passages: [] }
  }
